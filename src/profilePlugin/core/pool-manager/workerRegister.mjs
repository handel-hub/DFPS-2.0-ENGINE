    ...
    getStalledWorkers(maxSilenceMs) {
        ...
        return Array.from(busyIds)
            ...
            .filter(w => {
                ...
                // 1. Primary Check: Look for the last heartbeat.
                // 2. Secondary Fallback: Look for the exact millisecond the task was assigned.
                const lastActivity = metadata.lastHeartbeatAt || metadata.assignedAt;

                if (!lastActivity || !Number.isFinite(lastActivity)) {
                    console.warn(`[Register] Worker ${w.workerId} lacks timeline data. Reaping as corrupted.`);
                    return true; 
                }

                return (now - lastActivity) > maxSilenceMs;
            })
